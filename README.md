...
> 한 주의 일기가 모여 만들어진 주간 이미지. 아래 색 띠가 그 주 하루하루의 색이다.
...
|                          캘린더 홈                           |                          일기 작성                           |                           작성 도움 질문                           |
| :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------------: |
|    <img src="docs/home.png" width="230" alt="캘린더 홈">     | <img src="docs/diary-write.png" width="230" alt="일기 작성"> | <img src="docs/writing-help.png" width="230" alt="작성 도움 질문"> |
| 하루의 색이 캘린더에 쌓이고, 3일 이상이면 주간 보상이 열린다 |            진입 시 타임스탬프가 자동으로 들어간다            |              쓴 내용을 바탕으로 이어 쓸 질문을 받는다              |

|                          성찰 질문                          |                          오늘의 색                           |                          경험조각                          |
| :---------------------------------------------------------: | :----------------------------------------------------------: | :--------------------------------------------------------: |
| <img src="docs/reflection.png" width="230" alt="성찰 질문"> | <img src="docs/today-color.png" width="230" alt="오늘의 색"> | <img src="docs/experience.png" width="230" alt="경험조각"> |
|        다 쓰고 나면 그날에 맞는 질문이 하나 따라온다        |         감정을 분석해 색 · 키워드 · 코멘트를 만든다          |          익명 조각을 주고받고 짧은 반응을 남긴다           |
...
<img src="docs/weekly-image.png" width="260" alt="주간 이미지 상세">

- 한 주에 일기 3개 이상이면 캘린더에 보상 배지가 활성화된다.
- 서버가 그 주의 색과 키워드를 모아 이미지를 생성하며, 생성 중에는 4초 간격으로 폴링한다. 이미지 아래 색 띠가 그 주 하루하루의 색이고, 그 아래에 제목 · 대표 키워드 · 감정 키워드가 붙는다.
- 생성 시점은 확정적이지 않다. 전주 일기를 아직 쓰고 있으면 생성이 보류됐다가 재시도되므로 비동기 결과로 다룬다.
- 생성이 실패해도 사용자에게는 실패를 노출하지 않는다. 화면은 "만들고 있어요"를 유지한 채 재생성을 요청하고, 새 보상 id가 발급되면 그쪽으로 조용히 옮겨가 폴링을 이어간다.
- `imageUrl`은 **15분 만료 presigned URL**이라 저장해두고 재사용하지 않는다. 로드 실패 시 상세를 다시 조회해 새 URL을 받는다.
